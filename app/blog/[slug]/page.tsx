  ...
  return (
    <main className="py-16">
      <div className="max-w-4xl mx-auto px-6">
        {/* Back Link */}
        <Link href="/blog" className="inline-flex items-center text-orange hover:underline mb-8 text-sm font-medium">
          ← Retour au blog
        </Link>
          ...
          {post.categories && post.categories.length > 0 && (
            <div className="flex flex-wrap gap-2 mb-4">
              {post.categories.map((cat, index) => (
                <span key={index} className="px-3 py-1 bg-orange/10 text-orange text-xs font-medium rounded-full">
                  {cat.label}
                </span>
              ))}
            </div>
          ...
          <div className="flex flex-wrap items-center gap-4 text-sm text-gray-500">
            {post.author?.name && (
              <span className="flex items-center gap-2">
                <span className="w-8 h-8 bg-orange/10 rounded-full flex items-center justify-center">
        ...
        <footer className="mt-12 pt-8 border-t border-gray-200">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
            ...
            <div className="text-sm text-gray-500">
              Vous avez des questions ? <Link href="/faq" className="text-orange hover:underline">Consultez notre FAQ</Link>
            </div>
          </div>
        </footer>
      </div>
