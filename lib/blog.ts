...
async function getSiteId(): Promise<string | null> {
    ...
    const { data: site, error } = await supabaseAdmin
        .from("sites").select("id").eq("domain", SITE_DOMAIN).single();
    if (error || !site) { console.error("Site not found for domain:", SITE_DOMAIN); return null; }
...
export async function getPublishedBlogPosts(limit?: number): Promise<BlogPost[]> {
    ...
    let query = supabaseAdmin.from("blog_posts").select(BLOG_LISTING_SELECT)
        .eq("site_id", siteId).eq("status", "published").not("published_at", "is", null)
...
export async function getBlogPostBySlug(slug: string): Promise<BlogPost | null> {
    ...
    const { data: post, error } = await supabaseAdmin.from("blog_posts").select(BLOG_DETAIL_SELECT)
        .eq("site_id", siteId).eq("slug", slug).single();
    if (!error && post) return normalizePost(post);
    const { data: candidates } = await supabaseAdmin.from("blog_posts").select("id, slug, translations")
        .eq("site_id", siteId).eq("status", "published").not("translations", "is", null);
    if (!candidates) return null;
    const matchedCandidate = candidates.find((candidate) => {
        if (!candidate.translations) return false;
        let translations = candidate.translations;
        if (typeof translations === "string") { try { translations = JSON.parse(translations); } catch { return false; } }
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        return Object.values(translations).some((t: any) => t.slug === slug);
    });
    if (!matchedCandidate) return null;
    const { data: fullPost } = await supabaseAdmin.from("blog_posts").select(BLOG_DETAIL_SELECT).eq("id", matchedCandidate.id).single();
...
export async function getAllCategories(): Promise<{ id: string; slug: string; label: string }[]> {
    const siteId = await getSiteId();
    if (!siteId || !supabaseAdmin) return [];
    const { data, error } = await supabaseAdmin.from("site_categories").select("id, slug, label").eq("site_id", siteId);
    if (error) return [];
    return data || [];
}
...
export async function getBlogPostsForSitemap(lang: string = "fr") {
    const siteId = await getSiteId();
    if (!siteId || !supabaseAdmin) return [];
    const { data: posts, error } = await supabaseAdmin.from("blog_posts")
    ...
    return posts.reduce((acc: any[], post) => {
        ...
        if (post.translations) {
            ...
            if (matchingKey) { isMatch = true; if (translations[matchingKey]?.slug) finalSlug = translations[matchingKey].slug; }
        }
